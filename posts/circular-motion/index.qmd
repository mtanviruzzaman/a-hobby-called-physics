...
We can think that the particle has "fallen" from Q to S---due to centripetal acceleration---to remain on the circular path. Since, Q to S is in the radial direction and in that direction the tangential velocity $\vec{v}$ has no component, we can consider the "fall" from Q to S as a free-fall starting from rest. So, QS = h = $\frac{1}{2}|\vec{a}|(\Delta t)^2$. Using Pythagoras in $\triangle$ OPQ:
$$
\begin{align}
OQ^2 &= OP^2 + PQ^2 \\
(R+h)^2 &= R^2 + d^2 \\
